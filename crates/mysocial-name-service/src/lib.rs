...
use mysocial_types::collection_types::VecMap;
use mysocial_types::dynamic_field::Field;
use mysocial_types::id::{ID, UID};
use mysocial_types::object::{MoveObject, Object};
...
const DEFAULT_TLD: &str = "myso";
const ACCEPTED_SEPARATORS: [char; 2] = ['.', '*'];
const MYSO_NEW_FORMAT_SEPARATOR: char = '@';
...
/// A MysocialNameServiceRegistration object to manage an SLD
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct MysocialNameServiceRegistration {
    pub id: UID,
    pub domain: Domain,
...
pub struct SubDomainRegistration {
    pub id: UID,
    pub nft: MysocialNameServiceRegistration,
}

/// Two different view options for a domain.
/// `At` -> `test@example` | `Dot` -> `test.example.myso`
...
/// Rust version of the Move mysocial::table::Table type.
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
impl Domain {
    ...
    /// E.g. `test.example.myso` -> `example.myso`
    ///
    ...
    pub fn format(&self, format: DomainFormat) -> String {
        ...
        format!("{}{}{}", labels.join(sep), MYSO_NEW_FORMAT_SEPARATOR, sld)
    }
}
...
impl NameServiceConfig {
    ...
    pub fn record_field_id(&self, domain: &Domain) -> ObjectID {
        ...
        mysocial_types::dynamic_field::derive_dynamic_field_id(
            self.registry_id,
...
/// E.g.  `example.myso` -> `.` | example*myso -> `@` | `example*myso` -> `*`
...
/// Converts @label ending to label{separator}myso ending.
///
/// E.g. `@example` -> `example.myso` | `test@example` -> `test.example.myso`
fn convert_from_new_format(s: &str, separator: &char) -> Result<String, NameServiceError> {
    let mut splits = s.split(MYSO_NEW_FORMAT_SEPARATOR);
...
pub type SuinsRegistration = MysocialNameServiceRegistration;
pub const SUI_NEW_FORMAT_SEPARATOR: char = MYSO_NEW_FORMAT_SEPARATOR;
...
mod tests {
    ...
    fn test_parent_extraction() {
        let mut name = Domain::from_str("leaf.node.test.myso").unwrap();

        assert_eq!(name.parent().to_string(), "node.test.myso");

        name = Domain::from_str("node.test.myso").unwrap();

        assert_eq!(name.parent().to_string(), "test.myso");
    }
    ...
    fn test_expirations() {
        ...
        let mut name = NameRecord {
            nft_id: mysocial_types::id::ID::new(ObjectID::random()),
    ...
    fn test_name_service_outputs() {
        assert_eq!("@test".parse::<Domain>().unwrap().to_string(), "test.myso");
        assert_eq!(
            "test.myso".parse::<Domain>().unwrap().to_string(),
            "test.myso"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "test.test.example.myso"
        );
        assert_eq!(
            "myso@myso".parse::<Domain>().unwrap().to_string(),
            "myso.myso.myso"
        );

        assert_eq!("@myso".parse::<Domain>().unwrap().to_string(), "myso.myso");
        ...
        assert_eq!(
            "test.test.myso".parse::<Domain>().unwrap().to_string(),
            "test.test.myso"
        );
        assert_eq!(
            "test.test.test.myso".parse::<Domain>().unwrap().to_string(),
            "test.test.test.myso"
        );
    }
    ...
    fn test_different_wildcard() {
        assert_eq!("test.myso".parse::<Domain>(), "test*myso".parse::<Domain>(),);

        assert_eq!("@test".parse::<Domain>(), "test*myso".parse::<Domain>(),);
    }
    ...
    fn test_invalid_inputs() {
        ...
        assert!(".".parse::<Domain>().is_err());
        assert!("@".parse::<Domain>().is_err());
        assert!("@inner.myso".parse::<Domain>().is_err());
        assert!("@inner*myso".parse::<Domain>().is_err());
        assert!("test@".parse::<Domain>().is_err());
        assert!("myso".parse::<Domain>().is_err());
        assert!("test.test@example.myso".parse::<Domain>().is_err());
        assert!("test@test@example".parse::<Domain>().is_err());
    }

    #[test]
    fn test_output_tests() {
        let mut domain = "test.myso".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.myso");
        assert!(domain.format(DomainFormat::At) == "@test");

        domain = "test.test.myso".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.myso");
        assert!(domain.format(DomainFormat::At) == "test@test");

        domain = "test.test.test.myso".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.test.myso");
        assert!(domain.format(DomainFormat::At) == "test.test@test");

        domain = "test.test.test.test.myso".parse::<Domain>().unwrap();
        assert!(domain.format(DomainFormat::Dot) == "test.test.test.test.myso");
