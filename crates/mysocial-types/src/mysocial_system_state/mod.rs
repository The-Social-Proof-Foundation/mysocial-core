...
use crate::mysocial_system_state::epoch_start_mysocial_system_state::EpochStartSystemState;
use crate::mysocial_system_state::mysocial_system_state_inner_v2::SuiSystemStateInnerV2;
use crate::versioned::Versioned;
use crate::{id::UID, MoveTypeTagTrait, MYSO_SYSTEM_ADDRESS, MYSO_SYSTEM_STATE_OBJECT_ID};
use anyhow::Result;
...
use std::fmt;
// TODO: Update this import once protocol config is renamed
use sui_protocol_config::{ProtocolConfig, ProtocolVersion};

use self::mysocial_system_state_inner_v1::{SuiSystemStateInnerV1, ValidatorV1};
use self::mysocial_system_state_summary::{SuiSystemStateSummary, SuiValidatorSummary};

pub mod epoch_start_mysocial_system_state;
pub mod mysocial_system_state_inner_v1;
pub mod mysocial_system_state_inner_v2;
pub mod mysocial_system_state_summary;

#[cfg(msim)]
mod simtest_mysocial_system_state_inner;
#[cfg(msim)]
use self::simtest_mysocial_system_state_inner::{
...
const MYSO_SYSTEM_STATE_WRAPPER_STRUCT_NAME: &IdentStr = ident_str!("SuiSystemState");

pub const MYSO_SYSTEM_MODULE_NAME: &IdentStr = ident_str!("sui_system");
...
pub const SUI_SYSTEM_STATE_WRAPPER_STRUCT_NAME: &IdentStr = MYSO_SYSTEM_STATE_WRAPPER_STRUCT_NAME;
pub const SUI_SYSTEM_MODULE_NAME: &IdentStr = MYSO_SYSTEM_MODULE_NAME;
#[cfg(msim)]
pub const SUI_SYSTEM_STATE_SIM_TEST_V1: u64 = MYSO_SYSTEM_STATE_SIM_TEST_V1;
#[cfg(msim)]
pub const SUI_SYSTEM_STATE_SIM_TEST_SHALLOW_V2: u64 = MYSO_SYSTEM_STATE_SIM_TEST_SHALLOW_V2;
#[cfg(msim)]
pub const SUI_SYSTEM_STATE_SIM_TEST_DEEP_V2: u64 = MYSO_SYSTEM_STATE_SIM_TEST_DEEP_V2;
...
pub fn get_mysocial_system_state_wrapper(
    object_store: &dyn ObjectStore,
...
pub fn get_sui_system_state_wrapper(
    ...
    get_mysocial_system_state_wrapper(object_store)
}

pub fn get_mysocial_system_state(object_store: &dyn ObjectStore) -> Result<SuiSystemState, SuiError> {
    let wrapper = get_mysocial_system_state_wrapper(object_store)?;
        ...
        MYSO_SYSTEM_STATE_SIM_TEST_V1 => {
            let result: SimTestSuiSystemStateInnerV1 =
                get_dynamic_field_from_store(object_store, id, &wrapper.version).map_err(
                    |err| {
                        SuiError::DynamicFieldReadError(format!(
                            "Failed to load mysocial system state inner object with ID {:?} and version {:?}: {:?}",
                            id, wrapper.version, err
                        ))
                    },
                )?;
...
pub fn get_sui_system_state(object_store: &dyn ObjectStore) -> Result<SuiSystemState, SuiError> {
    get_mysocial_system_state(object_store)
}
