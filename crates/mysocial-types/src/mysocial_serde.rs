...
// TODO: Once we rename the protocol config, update this import
use sui_protocol_config::ProtocolVersion;

use crate::{
    parse_myso_struct_tag, parse_myso_type_tag, DEEPBOOK_ADDRESS, MYSO_CLOCK_ADDRESS,
    MYSO_FRAMEWORK_ADDRESS, MYSO_SYSTEM_ADDRESS, MYSO_SYSTEM_STATE_ADDRESS,
};
...
pub struct MySoStructTag;

impl SerializeAs<StructTag> for MySoStructTag {
    fn serialize_as<S>(value: &StructTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let f = to_myso_struct_tag_string(value).map_err(S::Error::custom)?;
        f.serialize(serializer)
    }
}

// For backwards compatibility
pub type SuiStructTag = MySoStructTag;

const MYSO_ADDRESSES: [AccountAddress; 7] = [
    AccountAddress::ZERO,
    AccountAddress::ONE,
    MYSO_FRAMEWORK_ADDRESS,
    MYSO_SYSTEM_ADDRESS,
    DEEPBOOK_ADDRESS,
    MYSO_SYSTEM_STATE_ADDRESS,
    MYSO_CLOCK_ADDRESS,
];
/// Serialize StructTag as a string, retaining the leading zeros in the address.
pub fn to_myso_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    let mut f = String::new();
    // trim leading zeros if address is in MYSO_ADDRESSES
    let address = if MYSO_ADDRESSES.contains(&value.address) {
    ...
    if let Some(first_ty) = value.type_params.first() {
        write!(f, "<")?;
        write!(f, "{}", to_myso_type_tag_string(first_ty)?)?;
        for ty in value.type_params.iter().skip(1) {
            write!(f, ", {}", to_myso_type_tag_string(ty)?)?;
        }
        write!(f, ">")?;
    }
    Ok(f)
...
fn to_myso_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    match value {
        TypeTag::Vector(t) => Ok(format!("vector<{}>", to_myso_type_tag_string(t)?)),
        TypeTag::Struct(s) => to_myso_struct_tag_string(s),
...
pub fn to_sui_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    to_myso_struct_tag_string(value)
}

#[deprecated(since = "1.0.0", note = "Use to_myso_type_tag_string instead")]
pub fn to_sui_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    to_myso_type_tag_string(value)
}

impl<'de> DeserializeAs<'de, StructTag> for MySoStructTag {
    fn deserialize_as<D>(deserializer: D) -> Result<StructTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_myso_struct_tag(&s).map_err(D::Error::custom)
    }
}

pub struct MySoTypeTag;

impl SerializeAs<TypeTag> for MySoTypeTag {
    fn serialize_as<S>(value: &TypeTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s = to_myso_type_tag_string(value).map_err(S::Error::custom)?;
        s.serialize(serializer)
    }
}

impl<'de> DeserializeAs<'de, TypeTag> for MySoTypeTag {
    fn deserialize_as<D>(deserializer: D) -> Result<TypeTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_myso_type_tag(&s).map_err(D::Error::custom)
    }
}

// For backwards compatibility
pub type SuiTypeTag = MySoTypeTag;
